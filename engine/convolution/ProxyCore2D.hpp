  ...
  namespace engine
  {
    namespace convolution
    {
      ...
      public:
        ...
        ProxyCore2D(ProxyCore2D<T>&& proxyCore) noexcept = delete;

        ProxyCore2D& operator=(const ProxyCore2D<T>& proxyCore) = delete;

        ProxyCore2D& operator=(ProxyCore2D<T>&& proxyCore) noexcept = delete;
      ...
      template <typename T>
      void ProxyCore2D<T>::GenerateOutput() const noexcept
      {
        Core.GenerateOutput();
      }
      ...
      template <typename T>
      void ProxyCore2D<T>::Save(std::ostream& ostream) const
      {
        Core.Save(ostream);
      }
